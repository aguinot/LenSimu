...
def save_ngmix_data(ngmix_cat):
    ...
    for i, id_tmp in enumerate(obj_id):
        ...
        output_arr["NGMIX_N_EPOCH"][i] = ngmix_n_epoch[ind[i]]
        output_arr["NGMIX_MCAL_FLAGS"][i] = ngmix_mcal_flags[ind[i]]
        output_arr["NGMIX_MOM_FAIL"][i] = ngmix_mom_fail[ind[i]]
...
def save_detect_ngmix_data(input_cat):
    ...
    for i, id_tmp in enumerate(obj_id):
        ...
        if len(ind) < 1:
            continue
        detect_arr["RA"][i] = input_cat["ra"][ind[i]]
        detect_arr["DEC"][i] = input_cat["dec"][ind[i]]
        detect_arr["X_IMAGE"][i] = input_cat["x"][ind[i]]
        detect_arr["Y_IMAGE"][i] = input_cat["y"][ind[i]]
        detect_arr["A_IMAGE"][i] = input_cat["a"][ind[i]]
        detect_arr["B_IMAGE"][i] = input_cat["b"][ind[i]]
        detect_arr["ELONGATION"][i] = input_cat["elongation"][ind[i]]
        detect_arr["ELLIPTICITY"][i] = input_cat["ellipticity"][ind[i]]
        detect_arr["KRON_RAD"][i] = input_cat["kronrad"][ind[i]]
        detect_arr["FLUX_AUTO"][i] = input_cat["flux"][ind[i]]
        detect_arr["FLUXERR_AUTO"][i] = input_cat["flux_err"][ind[i]]
        detect_arr["FLUX_RADIUS"][i] = input_cat["flux_radius"][ind[i]]
        detect_arr["MAG_AUTO"][i] = input_cat["mag"][ind[i]]
        detect_arr["MAGERR_AUTO"][i] = input_cat["mag_err"][ind[i]]
        detect_arr["SNR_WIN"][i] = input_cat["snr"][ind[i]]
        detect_arr["FLAGS"][i] = input_cat["flag"][ind[i]]
        detect_arr["IMAFLAGS_ISO"][i] = input_cat["ext_flag"][ind[i]]
        detect_arr["CENTRAL_FLAGS"][i] = input_cat["central_flag"][ind[i]]
